...
# Function to edit docker-compose.yaml (node version)
edit_docker_compose () {
  local file=$1
  echo "Editing docker-compose.yaml: $file"

  # Replace node version in the image line
  sed -i "s|image: ritualnetwork/infernet-node:.*|image: ritualnetwork/infernet-node:$node_version|g" "$file"
...
# Apply changes to docker-compose.yaml
edit_docker_compose "$DOCKER_COMPOSE"
